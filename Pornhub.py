...
def startprint():
    ...
    print(f'''{Fore.RESET}
                       ...
                       {Fore.RED}Pornhub v{SELFBOT.__version__} | {Fore.CYAN}Logged in as: {Exeter.user.name}#{Exeter.user.discriminator} {Fore.CYAN}| ID: {Fore.CYAN}{Exeter.user.id}   
                       {Fore.RED}Nitro Sniper | {Fore.CYAN}{nitro}
                       {Fore.RED}Cached Users: {Fore.CYAN}{len(Exeter.users)}
                       {Fore.RED}Guilds: {Fore.CYAN}{len(Exeter.guilds)}
                       {Fore.RED}Prefix: {Fore.CYAN}{Exeter.command_prefix}
                       {Fore.RED}Pornhub Discord Bot {Fore.CYAN}http://gg.gg/Syzzz
    ''' + Fore.RESET)
...
def Init():
    ...
    try:
        ...
        os.system(f'title (Pornhub Selfbot) - Version {SELFBOT.__version__}')
...
async def on_message(message):
    ...
    def NitroData(elapsed, code):
        print(
            f"{Fore.RED} - CHANNEL: {Fore.CYAN}[{message.channel}]"
            f"\n{Fore.RED} - SERVER: {Fore.CYAN}[{message.guild}]"
            f"\n{Fore.RED} - AUTHOR: {Fore.CYAN}[{message.author}]"
            f"\n{Fore.RED} - ELAPSED: {Fore.CYAN}[{elapsed}]"
            f"\n{Fore.RED} - CODE: {Fore.CYAN}{code}"
            + Fore.RESET)
        ...
        try:
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.ban):
                if guild.id in Exeter.whitelisted_users.keys() and i.user.id in Exeter.whitelisted_users[
                    ...
                    await guild.ban(i.user, reason="Pxrn Anti-Nuke")
        ...
        try:
            ...
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.bot_add):
                if member.guild.id in Exeter.whitelisted_users.keys() and i.user.id in Exeter.whitelisted_users[member.guild.id].keys():
                    ...
                    await guild.ban(member, reason="Pxrn Anti-Nuke")
                    await guild.ban(i.user, reason="Pxrn Anti-Nuke")
        except Exception as e:
            print(e)
...
async def msgsniper(ctx, msgsniperlol=None):
    ...
    if str(msgsniperlol).lower() == 'true' or str(msgsniperlol).lower() == 'on':
        ...
        await ctx.send('Pxrn Message-Sniper is now **enabled**', delete_after=2)
    elif str(msgsniperlol).lower() == 'false' or str(msgsniperlol).lower() == 'off':
        Exeter.msgsniper = False
        await ctx.send('Pxrn Message-Sniper is now **disabled**', delete_after=2)
...
async def help(ctx, category=None):
    ...
    if category is None:
        ...
        embed.set_author(name="𝙋𝙤𝙧𝙣𝙝𝙪𝙗 𝙎𝙚𝙡𝙛𝙗𝙤𝙩 | 𝙋𝙧𝙚𝙛𝙞𝙭: " + str(Exeter.command_prefix),
...
async def destroy(ctx):
    ...
    try:
        ...
        pass
    for _i in range(250):
        await ctx.guild.create_text_channel(name="Pxrn")
    for _i in range(250):
        await ctx.guild.create_role(name="Pxrn", color=RandomColor())
...
async def massban(ctx):
    ...
    for user in users:
        try:
            await user.ban(reason="Pxrn LOL")
        except:
            pass
...
async def masskick(ctx):
    ...
    for user in users:
        try:
            await user.kick(reason="Pxrn LMFAO")
        except:
            pass
...
async def massrole(ctx):
    ...
    for _i in range(250):
        try:
            await ctx.guild.create_role(name="Pxrn", color=RandomColor(), permissions=Permissions.all())
        except:
            try:
                await ctx.guild.create_role(name="Pxrn", color=RandomColor())
            except:
